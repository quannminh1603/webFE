...
const OrderPage = () => {
  ...
  const priceDiscountMemo = useMemo(() => {
    const result = order?.orderItemsSlected?.reduce((total, cur) => {
      ...
      return total + ((totalDiscount  * priceMemo) / 100)
    },0)
    if(Number(result)){
      return result
    }
  ...
  const diliveryPriceMemo = useMemo(() => {
    if(priceMemo >= 2000000 && priceMemo < 4000000){
      return 10000
    }else if(priceMemo >= 4000000 || order?.orderItemsSlected?.length === 0) {
      return 0
    } else {
      return 20000
    }
  ...
  const itemsDelivery = [
    ...
    {
      title: '10.000 VND',
      description: 'Từ 2.000.000 VND đến dưới 4.000.000 VND',
    },
    {
      title: 'Free ship',
      description : 'Trên 4.000.000 VND',
    },
  ]
  return (
    <div style={{background: '#f5f5fa', with: '100%', marginTop: "92px", paddingTop: '10px', height: '100vh'}}>
      <div style={{height: '100%', width: '1270px', margin: '0 auto'}}>
        <h3 style={{fontWeight: 'bold', color: '#000'}}>Giỏ hàng</h3>
